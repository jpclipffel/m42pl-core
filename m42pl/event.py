...
class Event:
    ...
    @property
    def signature(self):
        return self._signature and self._signature or self.sign(fields=[])
    ...
    def sign(self, fields: list = []):
        ...
        """
        if len(fields):
            if isinstance(fields[0], BaseField):
                self._signature = hashlib.md5(''.join([ str(f._read(self)) for f in fields ]).encode()).hexdigest()
            else:
                self._signature = hashlib.md5(''.join([ str(Field(f)._read(self)) for f in fields ]).encode()).hexdigest()
        else:
